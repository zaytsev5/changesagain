...
$(document).ready(function(){
  ...
  $('#change-ticket').click(async ()=>{
    if(isClick == false){
        ...
        let dateGo ;
        const res = await fetch('http://192.168.1.4:5000/user/allTickets');
         tickets = await res.json();

        // const post =await  fetch(`http://192.168.1.4:5000/getPostDetails/${}`)
...
async function showModal(e){
  ...
  const res = await fetch(`http://192.168.1.4:5000/getPostDetails/${e.target.getAttribute("data-post")}`)
...
async function handleCancledTicket(){
    ...
    fetch('http://192.168.1.4:5000/user/cancle',{
       method: 'POST',
        headers:{
