...
async function  getApis(){
    const response = await fetch('http://192.168.1.4:5000/trip');
    const myJson = await response.json();
    trips = myJson;
}
async function  getCusId(){
    const response = await fetch('http://192.168.1.4:5000/customer/id');
    const myJson = await response.json();
    ids = myJson;
}
const removeField = () =>{
...
function getStringTripAndGo(){
    ...
    location.href =  `http://192.168.1.4:5000/mua-ve-${from}-${to}?d=${dateTrip}`;  
}
