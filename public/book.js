...
buttonCancle.addEventListener('click',()=>{
  let quit = confirm("Mọi thông tin sẽ bị hủy. Bạn có chắc?")
    if(quit) return window.location = 'http://192.168.1.4:5000/home'
      return ;
})
...
timer.addEventListener('change',() =>{
    ...
    if(new Date(inputDate.value).toLocaleDateString() == new Date().toLocaleDateString()){
      if(timer.value.slice(0,2) - new Date().toLocaleTimeString().slice(0,2) >= 2){
...
inputDate.addEventListener('change',() =>{
     location.href =`http://192.168.1.4:5000${window.location.pathname}?d=${inputDate.value}#step1`
...
async function  findPostDate(){
    ...
    try{
      const response = await fetch(`http://192.168.1.4:5000/post/${tripID}/${dateTrip}`);
...
async function fillTimePicker(){
    ...
    let dateTrip = inputDate.value;
   try{ 
    const response = await fetch(`http://192.168.1.4:5000/time/${tripID}/${dateTrip}`);
       const myJson = await response.json();
...
async function findPostTime(){
   ...
   try{
       const response = await fetch(`http://192.168.1.4:5000/post/${tripID}/${dateTrip}/${time}`);
       const myJson = await response.json();
       const options = timer.options;
       postSelected = options[options.selectedIndex].id
   
       //GET NOIXUATPHAT 
       const post = await fetch(`http://192.168.1.4:5000/getPostDetails/${postSelected}`)
     ...
     console.log(postSelected)
      ...
      // location.href=`http://192.168.1.4:5000/mua-ve-saigon-kiengiang?d=${dateTrip}#step2`
...
try{  
  let response =  await fetch(`http://192.168.1.4:5000/checkseat/${postSelected}`)
 ...
 fetch('http://192.168.1.4:5000/customer/insert',{
        method: 'POST',
        headers:{
